...
public class WorldPlacementHandler {
    ...
    private static void spawnVoidPlatform(World world, BlockPos pos) {
        if (world.getBlockState(pos).getBlock() != ModBlocks.PORTAL_BLOCK) {
            BlockState platformBlock = ModBlocks.FIREDELTA_BRICKS.getDefaultState();
            for (int x = -3; x < 4; x++) {
                for (int z = -3; z < 4; z++) {
            ...
            world.setBlockState(pos, ModBlocks.FIREDELTA_BRICKS.getDefaultState());
